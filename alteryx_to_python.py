...
# Using wildcards in SELECT statement WHERE clauses
...
# Finding records where a field matches one of a list of values (the IN operator)
...
# Finding records where a field is between two values (the BETWEEN operator)
...
# Aliasing fields (using the AS command)
...
# Joining two tables together (JOIN)
...
# Unioning multiple tables together (UNION and UNION ALL)
...
# Comparing values against data in another table (WHERE EXISTS, ANY and ALL)
...
# Add rows into an existing table (INSERT INTO)
...
# Create a new table from values in an existing table (SELECTING INTO)
...
# Update values in a table (UPDATE)
